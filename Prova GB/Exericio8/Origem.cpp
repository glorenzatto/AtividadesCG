...
int memoryCardVerticesSize = 0, MEMORY_CARD_MOVE_KEY = GLFW_KEY_2;
...
{
	glfwInit();
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Tarefa GB - Gabriel Tamujo & Gustavo Lorenzatto", nullptr, nullptr);
	...
	GLuint texShield = loadTexture(shieldProps["map_Kd"]);

	GLuint memoryCardVAO = loadObj("../3d-models/memory-card/MemoryCard.obj", memoryCardVerticesSize);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glBindTexture(GL_TEXTURE_2D, texShield);
		...
		glDrawArrays(GL_TRIANGLES, 0, memoryCardVerticesSize);
